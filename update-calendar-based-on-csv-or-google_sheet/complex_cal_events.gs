...
function pushToCalendar() {
  ...
  //calendar variables
  var calendar = CalendarApp.getCalendarById('insert_cal_id_here')
  ...
  for (var i = 0; i < values.length; i++) {     
    ...
    if (values[i][0].length > 0) {
      ...
      if (values[i][2] != 'y') {                       
        ...
        var newEventTitle = 'NAME_HERE: ' + values[i][0];
        var setDescription = values[i][3];
        var setLocation = values[i][4];
        var newEvent = calendar.createAllDayEvent(newEventTitle, values[i][1],{description: values[i][3], location: values[i][4],});
